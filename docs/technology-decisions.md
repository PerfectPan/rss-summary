...
- The Daily AI Digest uses a two-phase model-editor protocol with a bounded research step between phases: `collect` exposes normalized evidence, the Agent may call `research-article` for selected public URLs, and `render` accepts structured `{category, headline, refs}` records plus URL-matched research evidence. The validator rejects unknown references, unrelated entities and unsupported numeric claims before Markdown exists; invalid drafts use deterministic source-grounded fallback headlines.
